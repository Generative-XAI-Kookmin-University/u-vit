...
def sample(config):
    ...
    # Create output directory and sample images
    output_path = config.sample.path
    if accelerator.is_main_process:
        os.makedirs(output_path, exist_ok=True)
    
    logging.info(f'Sampling {config.sample.n_samples} images to {output_path}...')
    utils.sample2dir(accelerator, output_path, config.sample.n_samples, config.sample.mini_batch_size, sample_fn, dataset.unpreprocess)
    
    if accelerator.is_main_process:
        logging.info(f'Successfully saved {config.sample.n_samples} images to {output_path}')
...
config_flags.DEFINE_config_file(
    "config", None, "Sampling configuration.", lock_config=False)
flags.mark_flags_as_required(["config"])
flags.DEFINE_string("nnet_path", None, "The nnet to sample from.")
